...
if config_parser.has_section('s3fuse_sys_admin'):
...
contego_vault_opts = [
    ...
    cfg.StrOpt('vault_s3_max_pool_connections',
               default='500',
               help='The maximum number of connections to keep in a connection pool')
]
...
def option_config():
    ...
    options['vault_s3_max_pool_connections'] = CONF.vault_s3_max_pool_connections
...
class BackendRepository(ObjectRepository):
    ...
    def object_open(self, object_name, flags):
        ...
        with self.__manifest_lock[object_name]:
            if not object_name in self.manifest:
                ...
                if (
                    ...
                    try:
                        ...
                        self.manifest[object_name]['open_handles'][fh] = {'readonly': False}
        ...
        # A vexing problem that causes cache file removed but some outstanding file handles
        # in the manifest[object_name]. As a temporary work around, we will touch the file
        ...
        fh = os.open(full_path, flags)
        try:
            self.manifest[object_name]['open_handles'][fh] = {'readonly': readonly}
    ...
    def _object_close(self, object_name, fh):
        ...
        with self.__manifest_lock[object_name]:
            try:
                ...
                if not self.manifest[object_name]['open_handles'][fh]['readonly']:
                    offset = 0
                    total_size = 0
                    segment_size = 0
                    while True:
    ...
    def rename(self, old, new):
        assert self.manifest.get(new, None) == None
